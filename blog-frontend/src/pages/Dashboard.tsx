...
const Dashboard = () => {
  ...
  const [animatedLike, setAnimatedLike] = useState<string | null>(null);
  const [animatedBookmark, setAnimatedBookmark] = useState<string | null>(null);
  ...
  const handleComment = async (id: string) => {
    ...
    try {
      await api.post(`/posts/${id}/comment`, { content }, { headers: { Authorization: `Bearer ${token}` } });
