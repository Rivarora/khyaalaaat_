...
const SkeletonCard = ({ isLight }: { isLight: boolean }) => (
  <div className={`p-8 rounded-3xl border animate-pulse min-h-[280px] flex flex-col gap-4 ${
    isLight
      ? "bg-white/50 border-pink-100"
      : "bg-white/5 border-white/10"
  }`}>
    <div className={`h-3 w-16 rounded-full ${isLight ? "bg-pink-200" : "bg-white/10"}`} />
    <div className={`h-6 w-3/4 rounded-lg ${isLight ? "bg-pink-200" : "bg-white/10"}`} />
    <div className="flex flex-col gap-2 flex-1">
      <div className={`h-3 w-full rounded ${isLight ? "bg-pink-100" : "bg-white/5"}`} />
      <div className={`h-3 w-5/6 rounded ${isLight ? "bg-pink-100" : "bg-white/5"}`} />
      <div className={`h-3 w-4/6 rounded ${isLight ? "bg-pink-100" : "bg-white/5"}`} />
      <div className={`h-3 w-5/6 rounded ${isLight ? "bg-pink-100" : "bg-white/5"}`} />
      <div className={`h-3 w-3/6 rounded ${isLight ? "bg-pink-100" : "bg-white/5"}`} />
    </div>
    <div className="flex justify-center gap-10 mt-2">
      <div className={`h-4 w-10 rounded ${isLight ? "bg-pink-200" : "bg-white/10"}`} />
      <div className={`h-4 w-8 rounded ${isLight ? "bg-pink-200" : "bg-white/10"}`} />
      <div className={`h-4 w-10 rounded ${isLight ? "bg-pink-200" : "bg-white/10"}`} />
    </div>
  </div>
);
...
const Dashboard = () => {
  ...
  const genres = [
    ...
    "Nature", "Friendship", "Parents", "Other",
  ];
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editId) {
        await api.put(`/posts/${editId}`, { title, content, genre }, { headers: { Authorization: `Bearer ${token}` } });
        toast.success("Poem updated ✨");
      } else {
        await api.post("/posts", { title, content, genre }, { headers: { Authorization: `Bearer ${token}` } });
        toast.success("Poem published 🌙");
      }
      setIsOpen(false); setEditId(null); setTitle(""); setContent(""); setGenre("Other");
      fetchPosts();
    ...
    <Layout>
      <div className={`relative min-h-full py-10 px-4 overflow-hidden transition-all duration-700 ${bgGradient}`}>
        ...
        <div className="text-center mb-16 relative">
          ...
          {decoded?.role === "admin" && (
            <button onClick={() => { setEditId(null); setTitle(""); setContent(""); setGenre("Other"); setIsOpen(true); }} className={`mt-8 px-8 py-3 rounded-full shadow-lg transition-all duration-300 ${publishBtn}`}>
              + Publish New Poem
            </button>
          )}
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-6">
          {genres.map((g) => (
            <button key={g} onClick={() => setActiveGenre(g)} className={`px-5 py-2 rounded-xl text-sm transition-all duration-300 ${activeGenre === g ? genreActive : genreInactive}`}>
              {g}
            </button>
          ))}
        </div>
        ...
        {loading && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 max-w-7xl mx-auto px-6 mb-10">
            {[...Array(6)].map((_, i) => (
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <motion.div initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className={`p-10 rounded-3xl w-full max-w-lg shadow-2xl ${isLight ? "bg-gradient-to-br from-pink-50 to-purple-50 text-gray-800 border border-pink-200" : "bg-gray-900 text-purple-200"}`}>
            <h3 className={`text-2xl mb-8 font-serif ${isLight ? "text-purple-700" : "text-purple-200"}`}>
              {editId ? "Edit Poem" : "Publish Poem"}
            </h3>

            <input type="text" placeholder="Title" value={title} onChange={(e) => setTitle(e.target.value)} className={`w-full p-3 mb-5 rounded-xl ${isLight ? "bg-white border border-pink-200 text-gray-800 placeholder-pink-300" : "bg-gray-800 text-white"}`} />

            <select value={genre} onChange={(e) => setGenre(e.target.value)} className={`w-full p-3 mb-5 rounded-xl ${selectClass}`}>
              {genres.filter((g) => g !== "All").map((g) => (
                <option key={g} value={g}>{g}</option>
              ))}
            </select>

            <textarea placeholder="Write your poem..." value={content} onChange={(e) => setContent(e.target.value)} className={`w-full p-3 mb-8 rounded-xl h-48 ${isLight ? "bg-white border border-pink-200 text-gray-800 placeholder-pink-300" : "bg-gray-800 text-white"}`} />

            <div className="flex justify-end gap-4">
              <button onClick={() => setIsOpen(false)} className={`px-5 py-2 rounded-xl ${isLight ? "bg-pink-100 hover:bg-pink-200 text-gray-700" : "bg-gray-700 hover:bg-gray-600 text-white"}`}>Cancel</button>
              <button onClick={handleSubmit} className={`px-6 py-2 rounded-xl text-white ${isLight ? "bg-gradient-to-r from-fuchsia-500 to-pink-500 hover:from-fuchsia-600 hover:to-pink-600" : "bg-purple-600 hover:bg-purple-700"}`}>
