...
const getRequestId = (request: RequestItem) => request._id || request.id || "";
...
export default function MyRequests() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setData((prev) => prev.filter((item) => getRequestId(item) !== id));
        ...
        {data.map((r) => {
          ...
          >
            ...
            <p className={`text-sm ${isLight ? "text-gray-500" : "text-gray-300"}`}>
                Requested: {new Date(r.createdAt || r.created_at || "").toLocaleString()}
            </p>
